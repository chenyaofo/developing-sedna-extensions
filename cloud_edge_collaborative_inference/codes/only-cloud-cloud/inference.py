...
class InferenceServer(BaseServer):  # pylint: disable=too-many-arguments
    ...
    def predict(self, data: InferenceItem):
        get_edge_time = time.time()
        upload_use_time = get_edge_time-data.uploadtime
        cloud_start_time = time.time()
        inference_res = self.model.inference(
            data.data, post_process=data.callback)
        cloud_stop_time = time.time()
        cloud_use_time = cloud_stop_time-cloud_start_time
        cloud_return_time = time.time()
        return ServePredictResult(result=inference_res,uploadusetime=upload_use_time,
                                  cloudusetime=cloud_use_time,cloudreturntime=cloud_return_time)